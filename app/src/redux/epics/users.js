...
// getJSON dependency is injected from epicMiddleware
export const fetchUserEpic = (action$, store, { getJSON }) => {
    ...
    return action$.ofType(FETCH_USER).mergeMap(action => {
        return getJSON(`https://api.github.com/users/${action.payload}`)
